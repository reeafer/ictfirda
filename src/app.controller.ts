...
export class AppController {
  ...
  @Post('events')
  async createEvent(
    @Body() body: { title: string; optionA: string; optionB: string },
  ) {
  ...
  async placeBet(
    @Request() req,
    @Body() body: { eventId: number; option: 'A' | 'B'; amount: number },
  ) {
    try {
      const username = req.user.username;
      return await this.appService.placeBet(
        body.eventId,
        body.option,
        body.amount,
        username,
  ...
  async playSlots(
    @Request() req,
    @Body() body: { gameId: number; betAmount: number },
  ) {
    try {
      const userId = req.user.userId;
      return await this.appService.playSlots(
        userId,
        body.gameId,
        body.betAmount,
      );
  ...
  async playBlackjack(
    ...
    try {
      const userId = req.user.userId;
      return await this.appService.playBlackjack(
        userId,
        body.gameId,
        body.betAmount,
      );
  ...
  async playRoulette(
    @Request() req,
    @Body()
    body: { gameId: number; betAmount: number; betType: string; betValue: any },
  ) {
    try {
      const userId = req.user.userId;
      return await this.appService.playRoulette(
        userId,
        body.gameId,
        body.betAmount,
        body.betType,
